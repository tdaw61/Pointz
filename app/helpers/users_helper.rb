module UsersHelper
  ...
  def avatar_for(object, type, css_class, size, css_id)
      ...
      case type
        when "small"
          image_tag(object.photo.picture.small_avatar.url.to_s, class: css_class, alt: object.name, size: size, id: css_id)
        when "medium"
          image_tag(object.photo.picture.medium_avatar.url.to_s, class: css_class, size: size, id: css_id)
        when "large"
          image_tag(object.photo.picture.large_avatar.url.to_s, class: css_class, size: size, id: css_id)
        when "thumb"
          image_tag(object.photo.picture.thumb.url.to_s, class: css_class, size: size, id: css_id)
        else
      ...
      case type
        ...
        when "thumb"
          image_tag('thumb_default_avatar.jpeg', class: css_class, id: css_id)
        else
          image_tag(PictureUploader.default_avatar.medium_avatar.url)
      end
    end
